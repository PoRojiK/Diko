...
const ProfileDropdown = ({ darkTheme, handleThemeToggle }) => {
    ...
    return (
        <div
          className={`absolute  top-full right-0 shadow-md p-4 rounded-lg border z-10 ${currentTheme.input.background} ${currentTheme.border} ${currentTheme.text} w-64`}
        >
          ...
          {/* Settings */}
          <div
            className={`p-2 rounded-lg cursor-pointer flex items-center mt-1 transition ${currentTheme.textSecondary} hover:${currentTheme.textHover}`}
          >
          ...
          <li
            ...
            className={`p-2 rounded-lg cursor-pointer flex justify-between items-center transition mt-1 ${currentTheme.textSecondary} hover:${currentTheme.textHover}`}
          >
    ...
    className={`absolute w-5 h-5 rounded-full border-2 transform transition-transform peer-checked:translate-x-[1.5rem] ${
      darkTheme
        ? 'bg-custom-black border-purple-600'
